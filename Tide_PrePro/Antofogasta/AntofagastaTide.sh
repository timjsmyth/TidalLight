...
start_line=$(grep -n $pattern $filename | tail -1) # Find first instance of the year/pattern match
echo "HEADER LINE" $start_line
start_row=${start_line:0:5} # Retrieve the row number of the first data recording for the year/pattern CAREFUL OF SINGLmE (0:1) AND DOUBLE DIGITS (0:2) 
echo $start_row
echo "Original file:"
head -5 $filename
